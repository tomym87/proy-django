...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = 'Por favor activa tu cuenta de app valtic'
            ...
            #messages.success(request, 'Se registro Usuario correctamente')
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.error(request, 'El link ha expidado')
        return redirect('login')
...
def change_password(request):
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            messages.error(request, 'El password no coincide con la confirmacion')
            return redirect('change_password')
